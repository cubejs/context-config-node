...
var ContextConfiguration = exports.ContextConfiguration = function(properties, contexts){
    ...
    _this._properties = []; //[{'_key_':'k', 'context':{'site':'en-US'}, 'value':'v']
    _this._columnFamily = {'_key_':{}};
    _this._knownContexts = contexts || [];

    _.each(properties, function(property){_this.append(property);});
};

ContextConfiguration.prototype.append = function(property){
    
    var _this = this,
        row = _this._properties.push(property) - 1,
        normalized = normalize(_this._knownContexts, property.context);

    normalized.unshift({'key':'_key_', 'value':property.key});

    _.each(normalized, function(context){
        
        var column = _this._columnFamily[context.key] || {},
            refs = column[context.value] || new BitSet();

        refs.set(row);
        column[context.value] = refs;
        _this._columnFamily[context.key] = column;
    });
};
...
ContextConfiguration.prototype.get = function(key, contextsIterator, onMiss){
    ...
    var _this = this,
        head = new BitSet().or(_this._columnFamily['_key_'][key] || miss),
        pick = head.nextSetBit(0);
    ...
    if(!_.has(contextsIterator, 'hasNext')){
        contextsIterator = _this.context(contextsIterator);
    }
    if(!contextsIterator.hasNext()){
        return _this._properties[pick].value;
    }

    for(var contexts = contextsIterator.next(); contexts; contexts = contextsIterator.next()){
        var copy = new BitSet().or(head),
            crossReferences = _.reduce(contexts, function(memoize, context){
                return memoize.and(_this._columnFamily[context.key][context.value] || miss);
            },
            copy);
